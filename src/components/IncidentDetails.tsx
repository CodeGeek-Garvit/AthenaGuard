...
export default function IncidentDetails({ incident, onBack }: { 
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      doc.text("FORENSIC DMCA NOTICE", 20, 28);
      ...
      doc.setFontSize(12);
      doc.setTextColor(0);
      doc.text(`INCIDENT ID: ${incident.id.toUpperCase()}`, 20, 45);
      doc.text(`TIMESTAMP: ${new Date().toLocaleString()}`, 20, 52);
      doc.text(`PLATFORM: ${incident.platform.toUpperCase()}`, 20, 59);
      
      doc.setFontSize(10);
      doc.text("VIOLATING URL:", 20, 70);
      doc.setTextColor(0, 0, 255);
      doc.text(incident.detectedUrl, 20, 75, { maxWidth: 170 });
      
      doc.setTextColor(0);
      doc.text(`SIMILARITY SCORE: ${(incident.similarityScore * 100).toFixed(2)}%`, 20, 90);
      doc.text(`TRANSFORMATIONS: ${incident.transformation.join(', ')}`, 20, 97);
      
      // Body
      doc.setFontSize(11);
      const bodyText = [
        "This document serves as formal notice of copyright infringement. The material above was detected ",
        "via AthenaGuard Automated Response Engine across monitored distribution channels.",
        "",
        "Digital Millennium Copyright Act Notification:",
        "1. Identification of Copyrighted Work - Star Sports Premium Broadcast Assets",
        "2. Identification of Infringing Material - Unauthorized transformation detected via Vector matching",
        "3. Verification of Identity - Star Sports Network / BCCI Rights Management Agent",
        "",
      ...
      doc.text("Generated by AthenaGuard Simulation Environment.", 20, 280);
      
      doc.save(`ATHENA_DMCA_${incident.id.split('_').pop()}.pdf`);
      
      detectionService.addNotification(`Forensic PDF for incident ${incident.id.split('_').pop()} exported to local storage.`);
    } catch (error) {
      console.error("PDF Generation failed:", error);
      detectionService.addNotification("PDF generation failed. Please try again.");
    } finally {
